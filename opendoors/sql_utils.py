...
def add_create_database(database_name: str, statements_with_defs: list):
    """
    ...
    :param statements_with_defs: existing SQL statements to apply to that database
    :return: amended list of statements
    """
    return [f"DROP DATABASE IF EXISTS `{database_name}`;",
            f"CREATE DATABASE `{database_name}`;",
            f"USE `{database_name}`;\n"] + statements_with_defs
...
def parse_remove_comments(original_db_sql: str, db_name: str = ''):
    """
    Remove comments and split into separate statements
    :param db_name: new database name
    :param original_db_sql: SQL statements
    :return: list of statements with new database CREATE and USE statements, and no comments
    """
    stmts = sqlparse.format(original_db_sql, None, strip_comments=True)
    raw_statements = sqlparse.split(stmts)
    clean_statements = [clean_up_sql_statement(item) for item in raw_statements if item not in [';', '#']]
    if db_name:
        return add_create_database(db_name, clean_statements)
    else:
